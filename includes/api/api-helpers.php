...
function asf_has_setting( $name = '' ) {
    return asf()->has_setting( $name );
}
...
function asf_raw_setting( $name = '' ) {
    return asf()->get_setting( $name );
}
...
function asf_update_setting( $name, $value ) {

    // validate name
    $name = asf_validate_setting( $name );

    // update
    return asf()->update_setting( $name, $value );
}


/**
 *  asf_validate_setting
 *
...
function asf_validate_setting( $name = '' ) {
    return apply_filters( 'asf/validate_setting', $name );
}
...
function asf_get_setting( $name, $value = null ) {

    // validate name
    $name = asf_validate_setting( $name );

    // check settings
    if ( asf_has_setting( $name ) ) {
        $value = asf_raw_setting( $name );
    }

    // filter
    $value = apply_filters( "asf/settings/{$name}", $value );
...
*  asf_append_setting
*
*  This function will add a value into the settings array found in the asf object
*
*  @type    function
*  @date    28/09/13
*  @since   5.0.0
...
function asf_append_setting( $name, $value ) {

    // vars
    $setting = asf_raw_setting( $name );
    ...
    // update
    return asf_update_setting( $name, $setting );
}
...
function asf_get_data( $name ) {
    return asf()->get_data( $name );
}
...
function asf_set_data( $name, $value ) {
    return asf()->set_data( $name, $value );
}
 ...
 *
 */
function asf_append_data( $name, $data ) {
    $prev_data = asf()->get_data( $name );
    if ( is_array( $prev_data ) ) {
        $data = array_merge( $prev_data, $data );
    }
    asf()->set_data( $name, $data );
}
...
function asf_has_done( $name ) {

    // return true if already done
    if ( asf_raw_setting( "has_done_{$name}" ) ) {
        return true;
    }

    // update setting and return
    asf_update_setting( "has_done_{$name}", true );
...
function asf_get_external_path( $file, $path = '' ) {
...
function asf_get_external_dir( $file, $path = '' ) {

    return asf_plugin_dir_url( $file ) . $path;
...
function asf_plugin_dir_url( $file ) {
...
function asf_parse_args( $args, $defaults = [] ) {

    // parse args
    $args = wp_parse_args( $args, $defaults );

    // parse types
    $args = asf_parse_types( $args );
...
function asf_parse_types( $array ) {
    return array_map( 'asf_parse_type', $array );
}
 ...
 *
 */
function asf_get_view( $view_path = '', $view_args = [] ) {
    // allow view file name shortcut
    if ( substr( $view_path, -4 ) !== '.php' ) {
        $view_path = asf_get_path( "includes/admin/views/{$view_path}.php" );
    }
...
function asf_merge_atts( $atts, $extra = [] ) {
...
function asf_nonce_input( $nonce = '' ) {

    echo '<input type="hidden" name="_asf_nonce" value="' . wp_create_nonce( $nonce ) . '" />';
...
function asf_extract_var( &$array, $key, $default = null ) {
...
function asf_extract_vars( &$array, $keys ) {

    $r = [];

    foreach ( $keys as $key ) {

        $r[ $key ] = asf_extract_var( $array, $key );
...
function asf_get_sub_array( $array, $keys ) {
...
function asf_get_post_types( $args = [] ) {

    // vars
    $post_types = [];

    // extract special arg
    $exclude   = asf_extract_var( $args, 'exclude', [] );
    $exclude[] = 'asf-field';
    $exclude[] = 'asf-field-group';
    ...
    // filter
    $post_types = apply_filters( 'asf/get_post_types', $post_types, $args );

    // return
    return $post_types;
}

function asf_get_pretty_post_types( $post_types = [] ) {

    // get post types
    if ( empty( $post_types ) ) {

        // get all custom post types
        $post_types = asf_get_post_types();
    ...
    foreach ( $post_types as $post_type ) {
        ...
        // vars
        $label = asf_get_post_type_label( $post_type );
...
function asf_get_post_type_label( $post_type ) {
...
*  asf_verify_nonce
*
*  This function will look at the $_POST['_asf_nonce'] value and return true or false
*
*  @type    function
*  @date    15/10/13
*  @since   5.0.0
...
function asf_verify_nonce( $value ) {

    // vars
    $nonce = asf_maybe_get_POST( '_asf_nonce' );
...
function asf_verify_ajax() {

    // bail early if not asf nonce
    if ( empty( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'asf_nonce' ) ) {
        return false;
    }

    // action for 3rd party customization
    do_action( 'asf/verify_ajax' );
...
function asf_get_image_sizes() {

    // vars
    $sizes = [
        'thumbnail' => __( 'Thumbnail', 'asf' ),
        'medium'    => __( 'Medium', 'asf' ),
        'large'     => __( 'Large', 'asf' ),
    ];
    ...
    // add full end
    $sizes['full'] = __( 'Full Size', 'asf' );

    // filter for 3rd party customization
    $sizes = apply_filters( 'asf/get_image_sizes', $sizes );
 ...
 *
 */
function asf_version_compare( $left = '', $compare = '>', $right = '' ) {
...
function asf_get_full_version( $version = '1' ) {
...
function asf_get_taxonomy_terms( $taxonomies = [] ) {

    // force array
    $taxonomies = asf_get_array( $taxonomies );

    // get pretty taxonomy names
    $taxonomies = asf_get_pretty_taxonomies( $taxonomies );
    ...
    foreach ( array_keys( $taxonomies ) as $taxonomy ) {
        ...
        foreach ( $terms as $term ) {
            ...
            $r[ $label ][ $k ] = asf_get_term_title( $term );
...
function asf_decode_taxonomy_terms( $strings = false ) {
    ...
    foreach ( $strings as $string ) {
        ...
        // vars
        $data     = asf_decode_taxonomy_term( $string );
...
function asf_decode_taxonomy_term( $value ) {
    ...
    // allow for term_id (Used by ASF v4)
    if ( is_numeric( $data['term'] ) ) {
        ...
        // attempt get term via ID (ASF4 uses ID)
        if ( ! $term ) {
...
function asf_get_array( $var = false, $delimiter = '' ) {
...
function asf_get_numeric( $value = '' ) {
...
function asf_get_posts( $args = [] ) {
    ...
    if ( ! $args['post_type'] ) {
        $args['post_type'] = asf_get_post_types();
    }
    ...
    if ( $args['post__in'] ) {
        ...
        $args['post__in'] = array_map( 'intval', asf_array( $args['post__in'] ) );
    }
...
function _asf_query_remove_post_type( $sql ) {
...
function asf_get_grouped_posts( $args ) {
    ...
    $post_types          = asf_get_array( $args['post_type'] );
    $post_types_labels   = asf_get_pretty_post_types( $post_types );
    ...
    if ( ! $is_single_post_type ) {
        add_filter( 'posts_orderby', '_asf_orderby_post_type', 10, 2 );
    }
    ...
    if ( ! $is_single_post_type ) {
        remove_filter( 'posts_orderby', '_asf_orderby_post_type', 10, 2 );
    }
    ...
    foreach ( $post_types as $post_type ) {
        ...
        if ( is_post_type_hierarchical( $post_type ) && empty( $args['s'] ) ) {
            ...
            $parent_id = asf_maybe_get( $args, 'post_parent', 0 );
            $offset    = 0;
...
function _asf_orderby_post_type( $ordeby, $wp_query ) {
...
function asf_get_post_title( $post = 0, $is_search = false ) {
    ...
    if ( $post->post_type !== 'attachment' ) {
        ...
        if( $is_search && !empty($ancestors) ) {
            ...
            // append
            $append .= ' | ' . __('Parent', 'asf') . ': ' . implode(' / ', $ancestors);
...
function asf_order_by_search( $array, $search ) {
    ...
    foreach ( array_keys( $array ) as $k ) {
        ...
        $array[ '_' . $k ] = asf_extract_var( $array, $k );
    ...
    foreach ( array_keys( $array ) as $k ) {
        ...
        $array[ substr( $k, 1 ) ] = asf_extract_var( $array, $k );
...
function asf_get_pretty_user_roles( $allowed = false ) {

    // vars
    $editable_roles = get_editable_roles();
    $allowed        = asf_get_array( $allowed );
    $roles          = [];
...
function asf_get_grouped_users( $args = [] ) {
    ...
    $min            = 0;
    $max            = 0;
    $users_per_page = asf_extract_var( $args, 'users_per_page' );
    $paged          = asf_extract_var( $args, 'paged' );
    ...
    // find array of post_type
    $user_roles = asf_get_pretty_user_roles( $args['role'] );
    ...
    if ( is_array( $args['role'] ) ) {
        ...
        // vars
        $roles = asf_extract_var( $args, 'role' );
    ...
    foreach ( $user_roles as $user_role_name => $user_role_label ) {
        ...
        foreach ( array_keys( $users ) as $key ) {
            ...
            // extract user
            $user = asf_extract_var( $users, $key );
...
function asf_str_exists( $needle, $haystack ) {
...
function asf_debug_start() {

    asf_update_setting( 'debug_start', memory_get_usage() );

}

function asf_debug_end() {

    $start = asf_get_setting( 'debug_start' );
...
function asf_encode_choices( $array = [], $show_keys = true ) {
...
function asf_decode_choices( $string = '', $array_keys = false ) {
...
function asf_str_replace( $string = '', $search_replace = [] ) {
...
function asf_split_date_time( $date_time = '' ) {

    // vars
    $php_date = asf_get_setting( 'php_to_js_date_formats' );
    $php_time = asf_get_setting( 'php_to_js_time_formats' );
...
function asf_convert_date_to_php( $date = '' ) {

    // vars
    $php_to_js = asf_get_setting( 'php_to_js_date_formats' );
    $js_to_php = array_flip( $php_to_js );

    // return
    return asf_str_replace( $date, $js_to_php );

}

/*
*  asf_convert_date_to_js
*
...
function asf_convert_date_to_js( $date = '' ) {

    // vars
    $php_to_js = asf_get_setting( 'php_to_js_date_formats' );

    // return
    return asf_str_replace( $date, $php_to_js );

}


/*
*  asf_convert_time_to_php
*
...
function asf_convert_time_to_php( $time = '' ) {

    // vars
    $php_to_js = asf_get_setting( 'php_to_js_time_formats' );
    $js_to_php = array_flip( $php_to_js );

    // return
    return asf_str_replace( $time, $js_to_php );

}


/*
*  asf_convert_time_to_js
*
*  This fucntion converts a date format string from PHP to JS
*
*  @type    function
...
function asf_convert_time_to_js( $time = '' ) {

    // vars
    $php_to_js = asf_get_setting( 'php_to_js_time_formats' );

    // return
    return asf_str_replace( $time, $php_to_js );

}


/*
*  asf_update_user_setting
*
*  description
*
*  @type    function
...
function asf_update_user_setting( $name, $value ) {

    // get current user id
    $user_id = get_current_user_id();

    // get user settings
    $settings = get_user_meta( $user_id, 'asf_user_settings', true );

    // ensure array
    $settings = asf_get_array( $settings );

    // delete setting (allow 0 to save)
    if ( asf_is_empty( $value ) ) {
    ...
    // update user data
    return update_metadata( 'user', $user_id, 'asf_user_settings', $settings );

}


/*
*  asf_get_user_setting
*
*  description
*
*  @type    function
...
function asf_get_user_setting( $name = '', $default = false ) {
...
function asf_in_array( $value = '', $array = false ) {
...
function asf_get_valid_post_id( $post_id = 0 ) {

    // allow filter to short-circuit load_value logic
    $preload = apply_filters( 'asf/pre_load_post_id', null, $post_id );
    if ( $preload !== null ) {
        return $preload;
    }
    ...
    if ( $post_id == 'options' ) {
        ...
        $dl = asf_get_setting( 'default_language' );
        $cl = asf_get_setting( 'current_language' );
    ...
    $post_id = apply_filters( 'asf/validate_post_id', $post_id, $_post_id );
...
function asf_get_post_id_info( $post_id = 0 ) {
    ...
    // if( asf_isset_cache($cache_key) ) return asf_get_cache($cache_key);
    ...
    if ( is_numeric( $post_id ) ) {
        ...
        // check if is taxonomy (ASF < 5.5)
        // - avoid scenario where taxonomy exists with name of meta type
        if ( ! in_array( $type, $meta ) && asf_isset_termmeta( $type ) ) {
            $type = 'term';
        }
    ...
    // update cache
    // asf_set_cache($cache_key, $info);

    // filter
    $info = apply_filters( 'asf/get_post_id_info', $info, $post_id );
...
/*
asf_log( asf_get_post_id_info(4) );

asf_log( asf_get_post_id_info('post_4') );

asf_log( asf_get_post_id_info('user_123') );

asf_log( asf_get_post_id_info('term_567') );

asf_log( asf_get_post_id_info('category_204') );

asf_log( asf_get_post_id_info('comment_6') );

asf_log( asf_get_post_id_info('options_lol!') );

asf_log( asf_get_post_id_info('option') );

asf_log( asf_get_post_id_info('options') );
...
function asf_isset_termmeta( $taxonomy = '' ) {
 ...
 *
 */
function asf_upload_files( $ancestors = [] ) {
    ...
    if ( empty( $_REQUEST['asf'][ $nonce_name ] ) || ! wp_verify_nonce( $_REQUEST['asf'][ $nonce_name ], 'asf/file_uploader_nonce/' . $field_key ) ) {
        return;
    }

    // Assign global _asfuploader for media validation.
    $_POST['_asfuploader'] = $field_key;

    // file found!
    $attachment_id = asf_upload_file( $file );

    // update $_POST
    array_unshift( $ancestors, 'asf' );
    asf_update_nested_array( $_POST, $ancestors, $attachment_id );
...
function asf_upload_file( $uploaded_file ) {
...
function asf_update_nested_array( &$array, $ancestors, $value ) {
    ...
    if ( isset( $array[ $k ] ) ) {
        ...
        return asf_update_nested_array( $array[ $k ], $ancestors, $value );
...
function asf_maybe_get( $array = [], $key = 0, $default = null ) {

    return isset( $array[ $key ] ) ? $array[ $key ] : $default;

}

function asf_maybe_get_POST( $key = '', $default = null ) {
...
function asf_maybe_get_GET( $key = '', $default = null ) {
 ...
 *
 */
function asf_get_attachment( $attachment ) {
    ...
    $response = apply_filters( 'asf/pre_load_attachment', null, $attachment );
    if ( $response !== null ) {
        return $response;
    }
    ...
    switch ( $type ) {
        case 'image':
            ...
            break;
        case 'video':
            $response['width']  = asf_maybe_get( $meta, 'width', 0 );
            $response['height'] = asf_maybe_get( $meta, 'height', 0 );
     ...
     *
     */
    return apply_filters( 'asf/load_attachment', $response, $attachment, $meta );
}
 ...
 *
 */
function asf_get_truncated( $text, $length = 64 ) {
    $text       = trim( $text );
...
/*
*  asf_current_user_can_admin
*
*  This function will return true if the current user can administrate the ASF field groups
*
*  @type    function
*  @date    9/02/2015
*  @since   5.1.5
...
function asf_current_user_can_admin() {

    if ( asf_get_setting( 'show_admin' ) && current_user_can( asf_get_setting( 'capability' ) ) ) {
...
function asf_get_filesize( $size = 1 ) {
...
function asf_format_filesize( $size = 1 ) {

    // convert
    $bytes = asf_get_filesize( $size );
...
function asf_get_valid_terms( $terms = false, $taxonomy = 'category' ) {

    // force into array
    $terms = asf_get_array( $terms );
...
function asf_validate_attachment( $attachment, $field, $context = 'prepare' ) {
    ...
    if ( $context == 'upload' ) {
        ...
        if ( strpos( $attachment['type'], 'image' ) !== false ) {
            ...
            $file['width']  = asf_maybe_get( $size, 0 );
            $file['height'] = asf_maybe_get( $size, 1 );
        ...
        $file['size']   = asf_maybe_get( $attachment, 'filesizeInBytes', 0 );
        $file['width']  = asf_maybe_get( $attachment, 'width', 0 );
        $file['height'] = asf_maybe_get( $attachment, 'height', 0 );
    ...
    if ( $file['width'] || $file['height'] ) {
        ...
        // width
        $min_width = (int) asf_maybe_get( $field, 'min_width', 0 );
        $max_width = (int) asf_maybe_get( $field, 'max_width', 0 );
        ...
        if ( $file['width'] ) {
            ...
            if ( $min_width && $file['width'] < $min_width ) {
                ...
                // min width
                $errors['min_width'] = sprintf( __( 'Image width must be at least %dpx.', 'asf' ), $min_width );

            } elseif ( $max_width && $file['width'] > $max_width ) {

                // min width
                $errors['max_width'] = sprintf( __( 'Image width must not exceed %dpx.', 'asf' ), $max_width );

            }
        }

        // height
        $min_height = (int) asf_maybe_get( $field, 'min_height', 0 );
        $max_height = (int) asf_maybe_get( $field, 'max_height', 0 );
        ...
        if ( $file['height'] ) {
            ...
            if ( $min_height && $file['height'] < $min_height ) {
                ...
                // min height
                $errors['min_height'] = sprintf( __( 'Image height must be at least %dpx.', 'asf' ), $min_height );

            } elseif ( $max_height && $file['height'] > $max_height ) {

                // min height
                $errors['max_height'] = sprintf( __( 'Image height must not exceed %dpx.', 'asf' ), $max_height );
    ...
    if ( $file['size'] ) {
        ...
        $min_size = asf_maybe_get( $field, 'min_size', 0 );
        $max_size = asf_maybe_get( $field, 'max_size', 0 );

        if ( $min_size && $file['size'] < asf_get_filesize( $min_size ) ) {

            // min width
            $errors['min_size'] = sprintf( __( 'File size must be at least %s.', 'asf' ), asf_format_filesize( $min_size ) );

        } elseif ( $max_size && $file['size'] > asf_get_filesize( $max_size ) ) {

            // min width
            $errors['max_size'] = sprintf( __( 'File size must not exceed %s.', 'asf' ), asf_format_filesize( $max_size ) );
    ...
    if ( $file['type'] ) {
        ...
        $mime_types = asf_maybe_get( $field, 'mime_types', '' );
        ...
        if ( ! empty( $mime_types ) && ! in_array( $file['type'], $mime_types ) ) {
            ...
            if ( count( $mime_types ) > 1 ) {
                ...
                $mime_types[] = $last2 . ' ' . __( 'or', 'asf' ) . ' ' . $last1;

            }

            $errors['mime_types'] = sprintf( __( 'File type must be %s.', 'asf' ), implode( ', ', $mime_types ) );
     ...
     *
     */
    $errors = apply_filters( "asf/validate_attachment/type={$field['type']}", $errors, $file, $attachment, $field, $context );
    $errors = apply_filters( "asf/validate_attachment/name={$field['_name']}", $errors, $file, $attachment, $field, $context );
    $errors = apply_filters( "asf/validate_attachment/key={$field['key']}", $errors, $file, $attachment, $field, $context );
    $errors = apply_filters( 'asf/validate_attachment', $errors, $file, $attachment, $field, $context );
...
add_filter( 'asf/settings/uploader', '_asf_settings_uploader' );

function _asf_settings_uploader( $uploader ) {
...
/*
function asf_translate_keys( $array, $keys ) {
...
function asf_translate( $string ) {

    // vars
    $l10n       = asf_get_setting( 'l10n' );
    $textdomain = asf_get_setting( 'l10n_textdomain' );
    ...
    if ( is_array( $string ) ) {
        ...
        return array_map( 'asf_translate', $string );
    ...
    if ( asf_get_setting( 'l10n_var_export' ) ) {
...
function asf_maybe_add_action( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ) {
...
function asf_is_row_collapsed( $field_key = '', $row_index = 0 ) {

    // collapsed
    $collapsed = asf_get_user_setting( 'collapsed_' . $field_key, '' );

    // cookie fallback ( version < 5.3.2 )
    if ( $collapsed === '' ) {

        $collapsed = asf_extract_var( $_COOKIE, "asf_collapsed_{$field_key}", '' );
        $collapsed = str_replace( '|', ',', $collapsed );

        // update
        asf_update_user_setting( 'collapsed_' . $field_key, $collapsed );
...
function asf_get_attachment_image( $attachment_id = 0, $size = 'thumbnail' ) {
...
function asf_get_post_thumbnail( $post = null, $size = 'thumbnail' ) {
    ...
    // vars
    $thumb_id  = $post->ID;
    $mime_type = asf_maybe_get( explode( '/', $post->post_mime_type ), 0 );
    ...
    $data['url'] = asf_maybe_get( $data['url'], 0 );
...
function asf_is_ajax( $action = '' ) {
    ...
    // check $action
    if ( $action && asf_maybe_get( $_POST, 'action' ) !== $action ) {
...
function asf_format_date( $value, $format ) {
 ...
 *  asf_dev_log
 *
 *  Used to log variables only if ASF_DEV is defined
 *
 * @date    25/8/18
 *
 * @param mixed
 ...
 *
 */
function asf_dev_log() {
    if ( defined( 'ASF_DEV' ) && ASF_DEV ) {
        call_user_func_array( 'asf_log', func_get_args() );
    }
}
...
*  asf_doing
*
*  This function will tell ASF what task it is doing
*
*  @type    function
*  @date    28/06/2016
*  @since   5.3.8
...
function asf_doing( $event = '', $context = '' ) {

    asf_update_setting( 'doing', $event );
    asf_update_setting( 'doing_context', $context );
...
*  asf_is_doing
*
*  This function can be used to state what ASF is doing, or to check
*
*  @type    function
*  @date    28/06/2016
*  @since   5.3.8
...
function asf_is_doing( $event = '', $context = '' ) {

    // vars
    $doing = false;

    // task
    if ( asf_get_setting( 'doing' ) === $event ) {
    ...
    // context
    if ( $context && asf_get_setting( 'doing_context' ) !== $context ) {
...
/*
*  asf_is_plugin_active
*
*  This function will return true if the ASF plugin is active
...
function asf_is_plugin_active() {

    // vars
    $basename = asf_get_setting( 'basename' );
...
function asf_send_ajax_results( $response ) {
...
function asf_is_sequential_array( $array ) {
...
*  @return  (boolean)
*/
function asf_is_associative_array( $array ) {
...
function asf_add_array_key_prefix( $array, $prefix ) {
...
function asf_remove_array_key_prefix( $array, $prefix ) {
...
function asf_strip_protocol( $url ) {
...
/*
*  asf_connect_attachment_to_post
*
...
*/
function asf_connect_attachment_to_post( $attachment_id = 0, $post_id = 0 ) {
     ...
     *
     */
    if ( ! apply_filters( 'asf/connect_attachment_to_post', true, $attachment_id, $post_id ) ) {
        return false;
    }
...
function asf_parse_markdown( $text = '' ) {
...
/**
 *  asf_convert_rules_to_groups
 *
 *  Converts an array of rules from ASF4 to an array of groups for ASF5
 *
 * @date    25/8/18
 *
 * @param array  $rules    An array of rules.
 * @param string $anyorall The anyorall setting used in ASF4. Defaults to 'any'.
 *
 * @return  array
 * @since   5.7.4
 *
 */
function asf_convert_rules_to_groups( $rules, $anyorall = 'any' ) {
    ...
    foreach ( $rules as $rule ) {
        ...
        // extract vars
        $group = asf_extract_var( $rule, 'group_no' );
        $order = asf_extract_var( $rule, 'order_no' );
 ...
 *
 */
function asf_register_ajax( $name = '', $callback = false, $public = false ) {

    // vars
    $action = "asf/ajax/$name";
 ...
 *
 */
function asf_str_camel_case( $string = '' ) {
    return lcfirst( str_replace( ' ', '', ucwords( str_replace( '_', ' ', $string ) ) ) );
}

/**
 *  asf_array_camel_case
 *
 ...
 *
 */
function asf_array_camel_case( $array = [] ) {
    $array2 = [];
    foreach ( $array as $k => $v ) {
        $array2[ asf_str_camel_case( $k ) ] = $v;
    }
